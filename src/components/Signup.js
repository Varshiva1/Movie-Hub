...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userData = {
        ...
        avatar: `https://ui-avatars.com/api/?name=${formData.name.trim()}&background=random&color=fff`,
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      dispatch(registerUser(googleUserData));
      navigate('/');
    } catch (error) {
  ...
  const handleFacebookSignup = async () => {
    ...
    try {
      ...
      dispatch(registerUser(facebookUserData));
      navigate('/');
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-violet-900 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="text-4xl font-bold text-white mb-2">Join MovieHub</h2>
